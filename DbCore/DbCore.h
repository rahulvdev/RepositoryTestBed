...
class Payload {
	public:
		...
		using Categories = std::vector<Item>;
		Payload() {}
		Payload(Categories categories, Item filePath, bool checkInStatus) :category_(categories), filePath_(filePath), open(checkInStatus) {}
		void category(Categories category) { category_ = category; }
		Categories& category() { return category_; }

		void filePath(std::string filePath) { filePath_ = filePath; }
		std::string& filePath() { return filePath_; }

		void checkInStatus(bool checkInStatus) { open = checkInStatus; }
		bool checkInStatus() { return open; }

	private:
		std::vector<Item> category_;
		Item filePath_;
		bool open;
	};
  ...
  public:
	...
	using VersionDb = std::unordered_map<Key, int>;
    using iterator = typename DbStore::iterator;
	using iteratorVer = typename VersionDb::iterator;
    ...
    size_t size();
	bool containsVersion(const Key& key);
	size_t getVersionNumber(const Key& key);
	size_t updateVersionNumber(const Key& key);
	bool insertFileVersionPair(const Key& key);
  ...
  template<typename T>
  bool DbCore<T>::containsVersion(const Key& key)
  {
	  iteratorVer iter = versionDb_.find(key);
	  return iter != versionDb_.end();
  }
  ...
  template<typename T>
  size_t DbCore<T>::getVersionNumber(const Key & key)
  {
	  if (!containsVersion(key))
		  throw std::exception("Version entry not found in Version table");
	  return versionDb_[key];
  }

  template<typename T>
  size_t DbCore<T>::updateVersionNumber(const Key & key)
  {
	  if (!containsVersion(key))
		  throw std::exception("Version entry not found in Version table");
	  return versionDb_[key]+=1;
  }


  //Member functions for updating, version table
  template<typename T>
  bool DbCore<T>::insertFileVersionPair(const Key & key)
  {
	  versionDb_[key] = 1;
	  return true;
  }
